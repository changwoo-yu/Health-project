...
## 버전 업 계획
* **관리자 페이지 기능**: 프로젝트의 관리자가 사용할 수 있는 페이지를 만들어, 필요한 데이터를 EXEL에 저장할 수 있고, 공지사항 작성 기능과 모든 게시글, 댓글의 삭제 권한을 추가할 계획입니다.
* **인바디 데이터 분석**: 사용자가 인바디 사진을 업로드하면, 해당 사진에서 데이터를 추출하고 이를 기반으로 개인화된 간단한 캐릭터를 생성하는 기능을 추가할 계획입니다. 또한, 인바디 데이터 변화를 그래프로 시각화하여 사용자에게 보여주는 기능을 계획하고 있습니다.

## 개선 목표
* **휴대폰 인증 기능**: 회원가입 페이지에 있는 휴대폰 인증 기능을 추가하여 보안을 강화할 예정입니다.
* **로그인 기능 개선**: 현재 아이디와 비밀번호를 통한 회원가입 방식 외에도, OAuth를 활용한 구글 및 카카오 간편 로그인 기능을 추가할 예정입니다.
* **게시글 반응 기능**: 각 게시글에 좋아요 같은 이모티콘을 추가하여 사용자들이 쉽게 반응할 수 있도록 개선할 계획입니다.
